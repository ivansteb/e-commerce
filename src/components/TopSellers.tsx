            ...
            {authors.map((author, index) => (
                <li key={index} className='flex items-center justify-between mb-4'>
                    <article className="flex justify-center items-center my-2">
                        <img src={author.image} alt={author.name} className="h-8 w-8 justify-center rounded-full mr-3" />
                        {/* Nombre del autor y botón si lo estoy siguiendo o no */}
                        <div className='flex flex-col gap-2'>
                            <p className='text-sm'>{author.name}</p>
                            <button className='text-xs w-[100px] bg-purple-800 text-purple-100'>Siguiendo</button>
                        </div>
                    </article>
                </li>
            ))}
